...
def gplmDCA_asymmetric(fastafile, outputfile, lambda_h, lambda_J, lambda_G,
    ...
    # Divide by 2 to keep the size of the coupling regularization equivalent to symmetric variant of plmDCA.
    ...
    CORRNORMS = np.subtract(
        NORMS
        , np.division(
            np.multiply(np.transpose(norm_means), norm_means)
            , norm_means_all))
